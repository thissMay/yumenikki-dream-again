...
func _unapply(_pl: Player) -> void:
	if action: (_pl as Player_YN).components.get_component_by_name("action_manager").cancel_action(
		(_pl as Player_YN).components.get_component_by_name("action_manager").action, _pl
	)
