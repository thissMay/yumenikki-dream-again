...
func _ready():
	...
	receiver = self.get_parent()

func _physics_update(_delta: float) -> void:
	if receiver != null && look_at_target != null:
		angle_rads = atan2(distance_to(look_at_target).y, distance_to(look_at_target).x)
		receiver.affector.position.x = clamp(receiver.affector.position.x, r, r * cos(angle_rads))
		receiver.affector.position.y = clamp(receiver.affector.position.y, r, r * sin(angle_rads))
