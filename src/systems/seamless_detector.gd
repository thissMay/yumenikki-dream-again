...
func _draw() -> void:
	if Engine.is_editor_hint():
		...
		var v_size := Vector2(boundary_size.x + 480 , clampf(min_boundary_size.y / 2 + tile_size.y * 3/2, 0, 300))
		var h_size := Vector2(clampf(min_boundary_size.x / 2 + tile_size.x * 2, 0, 500), min_boundary_size.y + clampf(min_boundary_size.y / 2 + tile_size.y * 3/2, 0, 300) * 7)
		
		var v_size_mirrored := Vector2(boundary_size.x - tile_size.x * 2, clampf(min_boundary_size.y / 2 + tile_size.y * 3/2, 0, 300))
		...
		if !right_disabled or !left_disabled:
			draw_rect(Rect2(Vector2(pos_right_overshoot.x - h_size.x, pos_right_overshoot.y), h_size), Color(Color.RED, .275)) # ---- right overshoot
			draw_rect(Rect2(pos_left_overshoot, h_size), Color(Color.CORNFLOWER_BLUE, .275)) # ---- left overshoot
			# ----> RIGHT
			
			draw_rect(Rect2(left.position.x + (tile_size.x * 3/2), left.position.y - boundary_size.y / 2, h_size.x, boundary_size.y), Color(Color.RED, .275)) # ---- right mirrored
			draw_rect(Rect2(right.position.x - (tile_size.x * 3/2), right.position.y - boundary_size.y / 2, -h_size.x , boundary_size.y), Color(Color.CORNFLOWER_BLUE, .275)) # ---- left mirrored
			...
			match _bound_side:
				bound_side.UP: 
					PLInstance.handle_player_world_warp(
						Vector2(_player.global_position.x, down.global_position.y + (tile_size.y * 3/2)), 
						PLInstance.get_pl().direction)
				bound_side.DOWN: 
					PLInstance.handle_player_world_warp(
						Vector2(_player.global_position.x, up.global_position.y - (tile_size.y * 3/2)), 
						PLInstance.get_pl().direction)
				bound_side.RIGHT: 
					PLInstance.handle_player_world_warp(
						Vector2(left.global_position.x + (tile_size.x * 3/2), _player.global_position.y), 
						PLInstance.get_pl().direction)
				bound_side.LEFT: 
					PLInstance.handle_player_world_warp(
						Vector2(right.global_position.x - (tile_size.x * 3/2), _player.global_position.y), 
						PLInstance.get_pl().direction)
